			...
			If txtcorreodocente.Text.Contains("@") Then
				If txtcorreodocente.Text.Contains(".") Then
					...
					Else
						...
						Dim job As HttpJob
						job.Initialize("", Me)
						job.PostString(ApiAddress,"INSERT INTO tblregistrodocente(nombreDocente, apellidoDocente, cedulaDocente, codigoDocente, correoDocente, contrasenaDocente) VALUES ('"&txtnombredocente.Text&"','"&txtapellidodocente.Text&"','"&txtceduladocente.Text&"','"&txtcodigodocente.Text&"','"&txtcorreodocente.Text&"','"&txtcondocente.Text&"')")
					End If
				Else
...
'					Dim ApiAddress As String = "http://192.168.1.2/schooltime.php"
'					Dim opc As HttpJob
'					opc.Initialize("",Me)
'					opc.Download2(ApiAddress,Array As String ("act","docente","nombredoc",txtnombredocente.Text,"ceduladoc",txtceduladocente.Text,"correodoc",txtcorreodocente.Text,"contrasenadoc",txtcondocente.Text))
'					Wait For(opc) JobDone(js As HttpJob)
'					If (js.Success) Then
'						Log("OK")
'						Log("Respuesta " & js.GetString)
'						MsgboxAsync("Su registro ha sido exitoso","Éxito de operacion")
'						hecho.Initialize("hecho", 2000)
'						hecho.Enabled= True
'					Else
'						Log(js.ErrorMessage)
